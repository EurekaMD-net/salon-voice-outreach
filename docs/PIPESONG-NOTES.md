...
- Full plan + diffs: `pipesong/docs/upgrade-flux-pipecat1x-2026-06-17.md`. **Status: APPLIED on
  branch `flux-pipecat1x` (commit `0a21cd6`, pushed; rebuilt from main via `/ship-it-py`,
  superseded `365d221`) — UNVALIDATED, NOT merged/deployed.**
...
- **Merge-watch (how the orchestrator learns it unblocks):** persistent cloud routine
  `trig_01XhS4LBYGvTynndAwwkvkx4` (daily 21:11 UTC / 3:11 PM CDMX), validated operational 2026-06-17 (returns
  `PENDING`). On merge → notifies + flags salon-voice-outreach coupled parts (webhook handler,
  agent-registration) UNBLOCKED. Manage/delete at claude.ai/code/routines.
